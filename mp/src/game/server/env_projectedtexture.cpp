...
BEGIN_DATADESC( CEnvProjectedTexture )
	...
	//DEFINE_AUTO_ARRAY_KEYFIELD( m_SpotlightTextureName, FIELD_CHARACTER, "texturename" ), // See https://developer.valvesoftware.com/wiki/Env_projectedtexture/fixes.
...
CEnvProjectedTexture::CEnvProjectedTexture( void )
{
	m_bState = true;
	m_flLightFOV = 45.0f;
	m_bEnableShadows = false; // NOTE(richard): Do we need to set this to true, so shadows would work from flashlight on server for other players? Answer: it does nothing.
